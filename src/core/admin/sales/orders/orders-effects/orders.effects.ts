...
export class OrdersEffects {
  constructor(private action$: Actions, private apiCli: OrdersService, protected appState: Store<store.AppState>) {}
  ...
  doOrderlists$: Observable<Action> = this.action$.pipe(
    ...
    switchMap(state => {
      return this.apiCli.getorderlist(state).pipe(
        switchMap(salesorders => [new actions.DoOrderSuccessAction(salesorders)]),
  ...
  doOrderdelete$: Observable<Action> = this.action$.pipe(
    ...
    switchMap(state => {
      return this.apiCli.getorderDelete(state).pipe(
        tap(data => {
          this.appState.dispatch(new layoutAction.GetTotalOrderCountAction({ count: true }));
          this.appState.dispatch(new layoutAction.GetTodayOrderCountAction({}));
          this.appState.dispatch(new layoutAction.GetTodayOrderAmountAction({}));
          this.appState.dispatch(new layoutAction.GetTotalOrderAmountAction({}));
        }),
        switchMap(salesorders => [new actions.DoOrderDeleteSuccessAction(salesorders)]),
  ...
  doOrderCount$: Observable<Action> = this.action$.pipe(
    ...
    switchMap(state => {
      return this.apiCli.getorderlist(state).pipe(
        switchMap(salesorders => [new actions.DoOrderCountSuccessAction(salesorders)]),
  ...
  doOrderDetails$: Observable<Action> = this.action$.pipe(
    ...
    switchMap(state => {
      return this.apiCli.getorderDetail(state).pipe(
        switchMap(orderDetails => [new actions.DoOrderDetailsSuccessAction(orderDetails)]),
  ...
  doOrderStatusChange$: Observable<Action> = this.action$.pipe(
    ...
    switchMap(state => {
      return this.apiCli.changeOrderStatus(state).pipe(
        switchMap(orderDetails => [new actions.DoOrderChangeStatusSuccess(orderDetails)]),
