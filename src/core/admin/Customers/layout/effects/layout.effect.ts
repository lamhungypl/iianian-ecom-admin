...
export class LayoutEffects {
  ...
  @Effect()
  doTotalCustomerListCount$: Observable<Action> = this.action$.pipe(
    ofType(actions.ActionTypes.GET_TOTAL_CUSTOMER_COUNT),
    map((action: actions.GetTotalCustomerCountAction) => action.payload),
    switchMap(state => {
      return this.layoutService.getCustomerListCount(state).pipe(
        switchMap(response => [new actions.GetTotalCustomerCountSuccessAction(response)]),
  ...
  doActiveCustomerListCount$: Observable<Action> = this.action$.pipe(
    ...
    switchMap(state => {
      return this.layoutService.getCustomerListCount(state).pipe(
        switchMap(response => [new actions.GetActiveCustomerCountSuccessAction(response)]),
  ...
  @Effect()
  doInActiveCustomerListCount$: Observable<Action> = this.action$.pipe(
    ofType(actions.ActionTypes.GET_INACTIVE_CUSTOMER_COUNT),
    map((action: actions.GetInActiveCustomerCountAction) => action.payload),
    switchMap(state => {
      return this.layoutService.getCustomerListCount(state).pipe(
        switchMap(response => [new actions.GetInActiveCustomerCountSuccessAction(response)]),
