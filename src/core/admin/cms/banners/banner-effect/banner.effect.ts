...
export class BannerEffect {
  constructor(private action$: Actions, private service: BannerService, protected appState: Store<store.AppState>) {}
  ...
  doBannerInActiveLists$: Observable<Action> = this.action$.pipe(
    ...
    switchMap(state => {
      return this.service.bannerList(state).pipe(
        switchMap(user => [new actions.DoBannerListInActiveSuccessAction(user)]),
        catchError(error => of(new actions.DoBannerListInActiveFailAction(error)))
      );
    })
  );
  ...
  doBannerPagination$: Observable<Action> = this.action$.pipe(
    ...
    switchMap(state => {
      return this.service.bannerPagiantion(state).pipe(
        switchMap(user => [new actions.DoBannerPaginationSuccessAction(new BannercountResponseModel(user))]),
  ...
  doAddBanner$: Observable<Action> = this.action$.pipe(
    ...
    switchMap(state => {
      ...
      return this.service.addBanner(state).pipe(
        switchMap(salesPayments => [new actions.DoBannerAddSuccessAction(salesPayments)]),
